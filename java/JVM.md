...
###类加载器的作用是什么
>类加载器实现类的加载动作，同时用于确定一个类。  
>对于任意一个类，都需要由加载它的类加载器和这个类本身一同确立其在Java虚拟机中的唯一性。  
>即使两个类来源于同一个Class文件，只要加载它们的类加载器不同，这两个类就不相等。  

###类加载器有哪些
- 启动类加载器（Bootstrap ClassLoader）
	使用C++实现（仅限于HotSpot），是虚拟机自身的一部分。负责将存放在\lib目录中的类库加载到虚拟机中。其无法被Java程序直接引用。

- 扩展类加载器（Extention ClassLoader）
	由ExtClassLoader实现，负责加载\lib\ext目录中的所有类库，开发者可以直接使用。
...
###类加载过程
[链接](http://blog.csdn.net/xuefeng0707/article/details/9132339)
...
- 准备  
	...
	但是，对于static final field，在此阶段是直接赋值的。
- 解析
...
>执行&ltclinit&gt()。
	...
	- &ltclinit&gt()中包含了一个类中所有类变量的赋值动作和静态块（写在static{ }里）的语句。执行顺序按在源文件中出现的顺序。
	- &ltclinit&gt()不需要调用父类的&ltclinit&gt()，这是由虚拟机来确保完成的。
	- 接口的&ltclinit&gt()中只包含接口中字段的赋值操作，父接口的&ltclinit&gt()不需要在子接口或实现类的&ltclinit&gt()之前执行，只有在真正引用到了父接口中定义的字段时，才会执行父接口的&ltclinit&gt()。  
	- &ltclinit&gt()是线程安全的。如果多个线程同时去初始化一个类，只有一个线程会去执行初始化，其他的线程都会阻塞。
